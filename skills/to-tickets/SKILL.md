...
When replacing an interface, let consumer and rollout evidence determine the ticket shape. If every consumer is controlled and the change can land atomically, keep the migration cohesive: update callers and tests and delete the old interface in the same ticket. If external consumers, mixed versions, independent deployments, or change width make an atomic migration unsafe, use expand–migrate–contract: introduce the new form beside the old, migrate consumers in independently safe batches, then remove the old form. Include a concrete removal condition for every temporary adapter and a final deletion ticket when the work is split. Do not assume an interface is internal or a special branching strategy without evidence.
