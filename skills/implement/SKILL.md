...
- When replacing an interface, establish who depends on it before choosing a migration. If every consumer is controlled in the same change and coordinated breakage is safe, migrate callers and tests and delete the old interface in the same wave. If external consumers, mixed versions, independent deployments, or staged rollout require compatibility, use expand–migrate–contract. Give every temporary adapter a concrete removal condition; do not call an interface internal without evidence.
